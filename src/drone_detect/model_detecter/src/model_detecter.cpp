...
std::array<float, 9>
Detect::processDetections(const yolo::BoxArray& detections, cv::Mat& image)
{
    ...
    // 取最高置信度框在图像中的ROI区域
    const int left =
        std::clamp(static_cast<int>(best->left), 0, image.cols);
    ...
    for (const auto& contour : contours) {
        ...
        contour_center_x.push_back(static_cast<float>(rect.x) +
                                   static_cast<float>(rect.width) * 0.5F);
    }

    std::vector<size_t> nearest_index(contours.size(), contours.size());
    std::vector<float>  nearest_x_diff(contours.size(),
    ...
    auto contourCenter = [](const std::vector<cv::Point>& contour) {
        const cv::Rect rect = cv::boundingRect(contour);
        return cv::Point2f(static_cast<float>(rect.x) +
                               static_cast<float>(rect.width) * 0.5F,
    ...
    if (pair_infos[left_pair].average_x >
        pair_infos[right_pair].average_x) {
