...
static __device__ float v8_item_at(float* predict, int num_bboxes,
                                   int output_cdim, int position, int attr,
                                   bool output_transposed)
{
    if (output_transposed) {
        return predict[attr * num_bboxes + position];
    }
    return predict[output_cdim * position + attr];
}
                                        ...
                                        bool output_transposed)
{
    ...
    float confidence =
        v8_item_at(predict, num_bboxes, output_cdim, position, 4,
                   output_transposed);
    int    label = 0;
    for (int i = 1; i < num_classes; ++i) {
        const float class_confidence =
            v8_item_at(predict, num_bboxes, output_cdim, position, 4 + i,
                       output_transposed);
        if (class_confidence > confidence) {
            confidence = class_confidence;
            label = i;
    ...
    float cx = v8_item_at(predict, num_bboxes, output_cdim, position, 0,
                          output_transposed);
    float cy = v8_item_at(predict, num_bboxes, output_cdim, position, 1,
                          output_transposed);
    float width = v8_item_at(predict, num_bboxes, output_cdim, position, 2,
                             output_transposed);
    float height = v8_item_at(predict, num_bboxes, output_cdim, position, 3,
                              output_transposed);
    float left = cx - width * 0.5f;
    float top = cy - height * 0.5f;
    float right = cx + width * 0.5f;
...
static void decode_kernel_invoker(float* predict, int num_bboxes,
                                  ...
                                  float* invert_affine_matrix,
                                  float* parray, int MAX_IMAGE_BOXES,
                                  Type type, bool output_transposed,
                                  cudaStream_t stream)
{
    auto grid = grid_dims(num_bboxes);
    auto block = block_dims(num_bboxes);

    if (type == Type::V8 || type == Type::V11 || type == Type::V8Seg) {
        checkKernel(decode_kernel_v8<<<grid, block, 0, stream>>>(
            ...
            MAX_IMAGE_BOXES, output_transposed));
...
public:
    ...
    bool               bbox_head_transposed_ = false;
              ...
              float confidence_threshold, float nms_threshold)
    {
        ...
        } else if (type == Type::V8 || type == Type::V11) {
            normalize_ =
                Norm::alpha_beta(1 / 255.0f, 0.0f, ChannelType::SwapRB);
            bbox_head_transposed_ =
                bbox_head_dims_[1] <= 256 && bbox_head_dims_[2] > bbox_head_dims_[1];
            num_classes_ =
                (bbox_head_transposed_ ? bbox_head_dims_[1] : bbox_head_dims_[2]) - 4;
            INFO("%s output layout: %s, dims=[%d,%d,%d], classes=%d",
                 type == Type::V11 ? "YOLOv11" : "YOLOv8",
                 bbox_head_transposed_ ? "attrs_boxes" : "boxes_attrs",
                 bbox_head_dims_[0], bbox_head_dims_[1], bbox_head_dims_[2],
                 num_classes_);
                                      ...
                                      void* stream = nullptr) override
    {
        ...
        for (int ib = 0; ib < num_image; ++ib) {
            ...
            const int decode_num_bboxes =
                bbox_head_transposed_ ? bbox_head_dims_[2] : bbox_head_dims_[1];
            const int decode_output_cdim =
                bbox_head_transposed_ ? bbox_head_dims_[1] : bbox_head_dims_[2];
            decode_kernel_invoker(
                image_based_bbox_output, decode_num_bboxes, num_classes_,
                decode_output_cdim, confidence_threshold_, nms_threshold_,
                affine_matrix_device, boxarray_device, MAX_IMAGE_BOXES,
                type_, bbox_head_transposed_, stream_);
        }
...
bool build_engine_from_onnx(const string& onnx_file,
                            const string& engine_file,
                            size_t workspace_size)
{
    if (!std::filesystem::exists(onnx_file)) {
        INFO("ONNX model does not exist: %s", onnx_file.c_str());
        return false;
    }
    ...
    if (!builder) {
        INFO("Failed to create TensorRT builder.");
        return false;
    }
    ...
    if (!network) {
        INFO("Failed to create TensorRT network.");
        return false;
    }
    ...
    if (!config) {
        INFO("Failed to create TensorRT builder config.");
        return false;
    }
    ...
    if (!parser) {
        INFO("Failed to create ONNX parser.");
        return false;
    }
    ...
    if (!parser->parseFromFile(
        ...
        INFO("Failed to parse ONNX model: %s", onnx_file.c_str());
        for (int i = 0; i < parser->getNbErrors(); ++i) {
            const auto* error = parser->getError(i);
            if (error) {
                INFO("ONNX parser error %d: %s", i, error->desc());
            }
        }
        return false;
    }
    ...
    if (!plan) {
        INFO("Failed to build TensorRT engine from ONNX: %s",
             onnx_file.c_str());
        return false;
    }

    const std::filesystem::path engine_path(engine_file);
    const auto parent = engine_path.parent_path();
    if (!parent.empty()) {
        std::error_code ec;
        std::filesystem::create_directories(parent, ec);
        if (ec) {
            INFO("Failed to create engine directory %s: %s",
    ...
    if (!out.is_open()) {
        INFO("Failed to open engine file for write: %s", engine_file.c_str());
        return false;
    }
    out.write(static_cast<const char*>(plan->data()), plan->size());
    if (!out.good()) {
        INFO("Failed to write TensorRT engine: %s", engine_file.c_str());
        return false;
    }

    INFO("Built TensorRT engine: %s", engine_file.c_str());
    return true;
}

shared_ptr<Infer<BoxArray>> load_or_build(const string& engine_file,
                                          const string& onnx_file,
                                          Type type,
                                          float confidence_threshold,
                                          float nms_threshold,
                                          size_t workspace_size)
{
    if (!std::filesystem::exists(engine_file)) {
        INFO("TensorRT engine does not exist, building from ONNX: %s",
             engine_file.c_str());
    ...
    return load(engine_file, type, confidence_threshold, nms_threshold);
}
